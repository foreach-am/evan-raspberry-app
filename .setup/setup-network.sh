...
sudo mv "$CONFIG_DTBO_FILE" "$CONFIG_OVERLAS"

FOUND="$(cat "$CONFIG_FILE" | grep dtparam | wc -l)"
if [[ "$FOUND" != "0" ]]; then
  sudo sed "s/^dtparam=spi/#dtparam=spi/g" -i "$CONFIG_FILE"
  sudo sed "s/^dtoverlay/#dtoverlay/g" -i "$CONFIG_FILE"
fi

sudo echo "dtparam=spi=on" | sudo tee -a "$CONFIG_FILE" > /dev/null
sudo echo "dtoverlay=w5500-spi1" | sudo tee -a "$CONFIG_FILE" > /dev/null
sudo echo "dtoverlay=spi1-1cs,cs1_pin=12" | sudo tee -a "$CONFIG_FILE" > /dev/null
